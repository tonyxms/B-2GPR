#Implement Algorithm 4 (sampling procedure for hmc-B^2GPR) for models with a non-constant fixed-effect term (vector-valued).
bayesian_bridge_sphmc_adp_hmc_convergence=function(x,y,prior_var,G,n_in,n_adpt,n_out_min,n_out_max,l,rou){
  n=nrow(x)
  d=ncol(x)
  d_beta=ncol(G)
  a_beta=prior_var[1]
    ...
    if (Iter>=n_out_min){
      if (Iter<n_out_max){
        if (Iter%%100==0){
          ...
          if (gelman.diag(combinedchains_beta)$mpsrf<1.1 & gelman.diag(combinedchains_omega)$mpsrf<1.1 & gelman.diag(combinedchains_tau_square)$psrf[1,1]<1.1 & gelman.diag(combinedchains_eta)$psrf[1,1]<1.1 & gelman.diag(combinedchains_v_beta_square)$psrf[1,1]<1.1 & gelman.diag(combinedchains_v_omega_square)$psrf[1,1]<1.1){
            convergence=TRUE
            break
          }
        }
  ...
  beta=apply(beta_record[ ,(n_adpt+1):dim(beta_record)[2]],1,median)
  ...
  beta_c=apply(beta_record_c[ ,(n_adpt+1):dim(beta_record_c)[2]],1,median)
