#Implement Algorithm 1 (Hamiltonian Monte Carlo) with adaptive step size tuning.
adp_HMC=function(q_cur, u_cur, du_cur, U,eps,eps_loghn,eps_An,eps_mu, L=5,iter,Nadpt){
  # initialization
  ...
  for (l in 1:randL){
    leap=leapfrog_HMC(q,v,u,du,U,eps)
    q=leap$q
    v=leap$v
    u=leap$u
    du=leap$du
  ...
  return (list(q=q,u=u,du=du,Ind=Ind,eps=eps,eps_loghn=eps_loghn,eps_An=eps_An))
