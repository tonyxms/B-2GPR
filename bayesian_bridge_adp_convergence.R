#Implement Algorithm 3 (sampling procedure for sph-B^2GPR) for models with a non-constant fixed-effect term (vector-valued).
bayesian_bridge_sphmc_adp_convergence=function(x,y,prior_var,G,n_in,n_adpt,n_out_min,n_out_max,q_norm,r){
    ...
    if (Iter>=n_out_min){
      if (Iter<n_out_max){
        if (Iter%%100==0){
          ...
          if (gelman.diag(combinedchains_beta)$mpsrf<1.1 & gelman.diag(combinedchains_omega)$mpsrf<1.1 & gelman.diag(combinedchains_tau_square)$psrf[1,1]<1.1 & gelman.diag(combinedchains_eta)$psrf[1,1]<1.1){
            convergence=TRUE
            break
          }
        }
